...
<?
	...
	for ($i=0; $i<$count; $i++)
	{
		...
		if (!$upfile_error[$i])
		{
			...
			//파일의 업로드 move_uploaded_flie( pc에 있는 임시파일명 , 실제 서버에 저장될 파일 경로)메소드
			// -> 리턴(true/false)
	...
	if ($mode=="modify")
	{
		...
		for ($i=0; $i<$count; $i++)					// update DB with the value of file input box
		{
			...
			if ($del_ok[$i] == "y") // 삭제 체크한 놈은
			{
				...
				$delete_name = $row[$delete_field]; //(날짜일시_번호).jpg
				
				$delete_path = "./data/".$delete_name; // ./data/(날짜일시_번호).jpg
		...
		$sql = "update $table set subject='$subject', content='$content', category='$category' where num=$num";
		mysql_query($sql, $connect);  // $sql 에 저장된 명령 실행
	}
	else
	{
		...
		$sql .= " file_name_0, file_name_1, file_name_2, file_name_3, file_copied_0,  file_copied_1, file_copied_2, file_copied_3, ";
		$sql .= " category) ";
		$sql .= "values('$userid', '$username', '$usernick', '$subject', '$content', '$regist_day', 0, '$is_html', ";
		$sql .= "'$upfile_name[0]', '$upfile_name[1]',  '$upfile_name[2]', '$upfile_name[3]', '$copied_file_name[0]', '$copied_file_name[1]',
		'$copied_file_name[2]','$copied_file_name[3]', ";
		$sql .= " '$category')";
		mysql_query($sql, $connect);  // $sql 에 저장된 명령 실행
	}
	...
	echo "
	   <script>
	    location.href = 'list.php?table=$table&page=$page&scale=$scale&mode2=$mode2';
	   </script>
	";
?>
